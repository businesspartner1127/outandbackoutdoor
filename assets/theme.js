  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _getVariantOptionList: function(variantOptions) {
      ...
      variantOptions.forEach(function(variantOption) {
        ...
        var variant_option = variantOption.name;
        var variant_value = variantOption.value;

        if (variant_option == 'size_and_size2') {
          variant_option = `<span style="text-transform: capitalize;">${variant_option.replace('_and_size2', '')}</span>`
          variant_value = `<span style="text-transform: uppercase;">${variant_value.split(':')[0]}</span>`
        } else if (variant_option == 'color') {
          variant_option = `<span style="text-transform: capitalize;">${variant_option}</span>`
          variant_value = `<span style="text-transform: capitalize;">${variant_value.split(':')[0]}</span>`
        } else if (variant_option == 'site_and_condition') {
          variant_option = `<span style="text-transform: capitalize;">${variant_option.replace('site_and_', '')}</span>`
          variant_value = `<span style="text-transform: capitalize;">${variant_value.split(':')[1]}</span>`
        }
